...
const adminLogin=TryCatch(async(req,res,next)=>{
    ...
    .cookie("speakEasy-admin-token",token,{
        ...cookieOptions,maxAge:1000*60*15
    })
    .json({
        success:true,
        message:"Authentocation Successfull, Welcome Boss"
    })
})
...
const allMessages=TryCatch(async(req,res)=>{
    ...
    const transformeMessages=messages.map(
...
const getDashboardStats=TryCatch(async(req,res)=>{
    ...
    last7DaysMessages.forEach(message=>{
        const indexApprox=(today.getTime()-message.createdAt.getTime())/(dayInMilisecond);
