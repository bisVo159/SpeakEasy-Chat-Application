...
const getMyGroups=TryCatch(async (req,res,next)=>{
    ...
    const groups=chats.map(({name,_id,members,groupChat})=>(
        {
            _id,groupChat,name,
...
const addMembers=TryCatch(async (req,res,next)=>{
    ...
    const allNewMembersPromise=members.map((i)=>User.findById(i,"name")) // onlys name return
    const allNewMembers=await Promise.all(allNewMembersPromise)

    const uniqMembers=allNewMembers.filter((i)=>!chat.members.includes(i._id.toString()))
                                        .map(i=>i._id)
...
const getChatDetails=TryCatch(async(req,res,next)=>{
    if(req.query.populate==="true"){
        ...
        // after lean chat is not more mondb object
...
const deleteChat=TryCatch(async(req,res,next)=>{
    ...
    // Here we have to delete all message as well as attachments from cloudinary
    const messageWithAttachments=await Message.find(
        {chat:chatId.toString(),attachments:{$exists:true,$ne:[]}})
